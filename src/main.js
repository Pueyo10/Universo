...
async function boot() {
  ...
  const nWarm = await engine.warmup([engine.scene, engine.nebulaScene, engine.volScene], [engine.blackHolePass.material, engine.taa.material, engine.taa.copyMaterial, engine.exposurePass.material], p => progress(0.9 + 0.1 * p, t('shaders')));
