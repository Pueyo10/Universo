...
export class TAAPass extends Pass {
  constructor(camera, width, height) {
    ...
    this.historyDepth = new THREE.WebGLRenderTarget(width, height, { type: THREE.FloatType, format: THREE.RedFormat, internalFormat: 'R32F', depthBuffer: false, stencilBuffer: false, minFilter: THREE.NearestFilter, magFilter: THREE.NearestFilter, generateMipmaps: false });
    ...
    Object.assign(this.material.uniforms, { tHistoryDepth: { value: this.historyDepth.texture }, uPrevScale: { value: new THREE.Vector2(1, 1) }, uPrevJitter: { value: new THREE.Vector2() }, uPrevLogFar: { value: 1 } });
    this.depthMaterial = new THREE.ShaderMaterial({ uniforms: { tDepth: { value: null }, uResolution: this.material.uniforms.uResolution }, vertexShader: vert, fragmentShader: /* glsl */`uniform sampler2D tDepth; uniform vec2 uResolution; void main() { gl_FragColor = vec4(texelFetch(tDepth, ivec2(gl_FragCoord.xy), 0).r, 0.0, 0.0, 1.0); }`, depthTest: false, depthWrite: false });
  ...
  render(renderer, writeBuffer, readBuffer) {
    ...
    // Save depth only over the rendered viewport: one nearest fetch per input pixel.
    this.historyDepth.viewport.set(0, 0, Math.round(this.width * this.scale.x), Math.round(this.height * this.scale.y));
    this.historyDepth.scissor.copy(this.historyDepth.viewport); this.historyDepth.scissorTest = true;
    renderer.setRenderTarget(this.historyDepth);
    this.depthMaterial.uniforms.tDepth.value = readBuffer.depthTexture;
    this.fsq.material = this.depthMaterial; this.fsq.render(renderer);
    u.uPrevScale.value.copy(this.scale); u.uPrevJitter.value.copy(this._jitter); u.uPrevLogFar.value = u.uLogFar.value;
    this._h = 1 - this._h;
  ...
  dispose() { for (const r of this.hist) r.dispose(); this.historyDepth.dispose(); this.depthMaterial.dispose(); this.material.dispose(); this.copyMaterial.dispose(); this.fsq.dispose(); }
}
