...
- **Moon and Mars globes**: colour maps from LRO (LROC WAC) and Viking, and tangent-space normal maps derived from the
  LOLA / MOLA DEMs (2K base, 4K with the 8K colour map), so craters, basins, Tharsis and Valles Marineris show relief
  along the terminator at orbital distances. The Moon uses lunar-Lambert photometry (McEwen): no limb darkening at full
  phase, Lambertian toward the terminator.
...
(CC BY 4.0), except the Moon and Mars globes (public domain, regenerated by `tools/planet_maps.py`):
Moon colour and relief from the NASA SVS [CGI Moon Kit](https://svs.gsfc.nasa.gov/4720) (Ernie Wright, NASA's
Scientific Visualization Studio: LRO WAC Hapke-normalised colour mosaic, LRO LOLA elevation); Mars colour from the
USGS Astrogeology Viking MDIM colour mosaic (`Mars_Viking_ClrMosaic_global_925m`, recoloured with measured spectra),
Mars relief from the MGS MOLA MEGDR (NASA PDS Geosciences Node). Close-up surface tiles: NASA GIBS (Blue Marble Next Generation, VIIRS)
